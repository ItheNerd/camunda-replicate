...
import { createBpmnIoEvent } from "@/lib/ivy-bpmn-modeler/events/bpmnio/BpmnIoEvents";
import { createContentSavedEvent } from "@/lib/ivy-bpmn-modeler/events/modeler/ContentSavedEvent";
import { createDmnViewsChangedEvent } from "@/lib/ivy-bpmn-modeler/events/modeler/DmnViewsChangedEvent";
import { createNotificationEvent } from "@/lib/ivy-bpmn-modeler/events/modeler/NotificationEvent";
import { createUIUpdateRequiredEvent } from "@/lib/ivy-bpmn-modeler/events/modeler/UIUpdateRequiredEvent";
...
const DmnEditor: React.FC<DmnEditorProps> = (props) => {
    ...
    async (event: string, data: any) => {
      ...
      if (event && CONTENT_SAVED_EVENT.indexOf(event) !== -1 && ref.current) {
        try {
          ...
          onEvent(
            createContentSavedEvent(saved.xml, undefined, "diagram.changed")
          );
        } catch (e) {
  ...
  const propertiesPanelContainer: ReactNode =
    propertiesPanelOptions?.container ?? (
      <div
        id="dmnprop"
        className={clsx(
